 ...
 */
function getDecoration(extensionUri: vscode.Uri): vscode.TextEditorDecorationType {
  if (decoration) {
    return decoration;
  }
...
function decorateHerokuCommands(context: vscode.ExtensionContext): void {
  ...
  const herokuCommandNodes = lexer.findAllNodeKinds(isHerokuCallExpression) as CallExpr[];
  ...
  for (const command of herokuCommandNodes) {
    const [, commandNameLit] = lexer.findAllNodeKinds(isLiteral, command) as Lit[];
  ...
  editor.setDecorations(getDecoration(context.extensionUri), decorationOptions);
}
